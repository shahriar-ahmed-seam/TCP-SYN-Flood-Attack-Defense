...
This project was carried out under the specifications of Topic 06 for CSE 406 \cite{buet_cse406}. The primary technical objectives were:
\begin{enumerate}
    ...
    \item \textbf{Bitwise Protocol Header Serialization:} Manually construct Layer 3 (IPv4) and Layer 4 (TCP) binary headers, implement random source IP and port generation, and implement RFC 1071 \cite{rfc1071} 16-bit one's complement checksums using RFC 793 \cite{rfc793} TCP pseudo-headers.
...
Under RFC 793 \cite{rfc793}, TCP establishes a bidirectional byte stream between two endpoints by synchronizing initial sequence numbers:
...
\begin{itemize}
    ...
    \item \textbf{Server Side:} Upon receiving a $SYN$, the server kernel allocates approximately 280 to 500+ bytes for a \texttt{struct request\_sock} \cite{linux_kernel}, registers hardware timer callbacks for exponential retransmissions, records routing information, and consumes a slot in its half-open socket backlog table.
\end{itemize}
Because the server commits substantial kernel state for every incoming $SYN$ while the sender commits none, an attacker transmitting moderate volumes of requests can rapidly exhaust the server's connection tracking capacity, causing Denial of Service as surveyed in RFC 4987 \cite{eddy2007}.

\section{The Linux Half-Open Connection Architecture}
In the Linux kernel network stack \cite{linux_kernel}, connection requests are processed through two distinct queues:
\begin{enumerate}
...
To prevent spoofed IP packets from escaping onto university or local network infrastructure, the experimental testbed was deployed entirely inside an isolated virtual network environment using Linux Network Namespaces and virtual Ethernet pairs (\texttt{veth}). The testbed comprises three distinct nodes connected through a virtual Layer 2 bridge on subnet \texttt{192.168.10.0/24}, as illustrated in Figure~\ref{fig:network_topology}.
...
To evaluate baseline vulnerability and subsequent defensive remediations, the victim's kernel parameters are configured in the Linux networking subsystem \cite{linux_kernel} via \texttt{sysctl} as detailed in Table~\ref{tab:sysctl}.
...
The custom IPv4 header occupies 20 bytes (no IP options), complying with RFC 791 \cite{rfc791}:
\begin{table}[H]
    \centering
    \small
...
The crafted TCP header occupies 20 bytes without options, complying with RFC 793 \cite{rfc793}:
\begin{table}[H]
    \centering
    \small
    \renewcommand{\arraystretch}{0.88}
...
The TCP checksum cannot be computed from the TCP segment alone. RFC 793 \cite{rfc793} defines a 12-byte conceptual \textbf{Pseudo-Header} to verify that routing properties match transport-layer data:
\begin{lstlisting}[style=customc]
...
The checksum algorithm sums all 16-bit integers in one's complement arithmetic as specified in RFC 1071 \cite{rfc1071} across the pseudo-header concatenated with the TCP header:
\begin{equation}
...
The core logic computes the RFC 1071 \cite{rfc1071} Internet Checksum across the pseudo-header and serialized TCP header:
...
Per course submission standards, the complete source code, execution scripts, and test harnesses are hosted in the project GitHub repository: \url{https://github.com/shahriar-ahmed-seam/TCP-SYN-Flood-Attack-Defense}.
